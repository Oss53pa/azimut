 ...
 *
 * Implementation uses the Weiler-Atherton algorithm for polygon clipping.
 * All coordinates are in meter-space.
 */
...
// ---------------------------------------------------------------------------
// Geometry primitives (no external library — A3.3)
// ---------------------------------------------------------------------------
...
function polygonSubtraction(subject: Polygon, clip: Polygon): Polygon {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const nextInside = pointInPolygon(next, clipCCW);
    ...
    // If transitioning inside→outside, add clip boundary points
    if (curInside && !nextInside) {
      // Already handled by intersection points above
    }
  }
